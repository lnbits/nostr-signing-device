...
def handle_rx(sender: int, data: bytearray):
    ...
    try:
        ...
        print(f"[DEBUG] Raw received data ({len(data)} bytes): {received!r}")
        ...
        while BATCH_DELIMITER in received:
            ...
            print(f"[DEBUG] Added batch to buffer (buffer size: {len(notification_buffer)} bytes): {notification_buffer!r}")
        ...
        if received:
            ...
            if BATCH_DELIMITER not in data.decode():
                print(f"[DEBUG] Final batch received, complete message ({len(notification_buffer)} bytes): {notification_buffer!r}")
                print(f"[#] {notification_buffer!r}")
                notification_buffer = ""  # Reset for next message
            else:
                print(f"[DEBUG] Added partial batch to buffer (buffer size: {len(notification_buffer)} bytes): {notification_buffer!r}")
